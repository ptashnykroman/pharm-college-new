...
import { HERO_PRIMARY_LINK, HERO_SECONDARY_LINK, NEWS_INDEX_PATH } from '@/shared/lib/site-config'
...
export type HomeHeroViewModel = HomePageViewModel['hero']
export type HomeMainSectionsViewModel = Omit<HomePageViewModel, 'hero'>
...
export function buildHomeHeroViewModel(heroData: GetHomeHeroQuery): HomeHeroViewModel {
  ...
  return {
    ...
    videoPoster: resolveImage(background?.video_poster_primary ?? null, 'hero', hero.title),
    frameUrl: background?.iframe_url || null,
    framePoster: resolveImage(background?.frame_poster ?? null, 'hero', hero.title),
    logoImage: resolveImage(hero.logo, 'logo', hero.title),
    announcements: (heroData.advertisements?.data ?? []).filter(hasAttributes).map((item) => ({
...
export function buildHomeMainSectionsViewModel(
  ...
  if (!about || !statBlock || !gallery || !contacts) {
    throw new Error('Homepage main sections CMS data is incomplete.')
  }
  ...
  return {
    ...
    gallery: {
      title: gallery.title,
      items: gallery.GalleryItems.filter((item): item is NonNullable<typeof item> => Boolean(item))
        .map((item) => {
    ...
    contacts: {
      ...
      people: contacts.Contacts.filter((item): item is NonNullable<typeof item> => Boolean(item)).map((item) => ({
        id: item.id,
        name: item.name,
    ...
    news: (newsData.novinas?.data ?? [])
      ...
      .map((item) => {
        ...
        const firstTag = (item.attributes.news_tags?.data ?? []).find((tag) => Boolean(tag.attributes?.title))

        return {
          id: item.id,
          title: item.attributes.title,
          excerpt: `${previewText.slice(0, 180).trim()}${previewText.length > 180 ? '...' : ''}`,
      ...
      .filter((item): item is NonNullable<typeof item> => Boolean(item)),
    events: (eventsData.events?.data ?? []).filter(hasAttributes).map((item) => ({
      id: item.id ?? `${item.attributes.title}-${item.attributes.date}`,
      title: item.attributes.title,
      date: formatDateParts(item.attributes.date),
      image: resolveImage(item.attributes.image, 'card', item.attributes.title),
    })),
    partners: (partnersData.partners?.data ?? []).filter(hasAttributes).map((item) => ({
...
export function buildHomePageViewModel(
  ...
  return {
    hero: buildHomeHeroViewModel(heroData),
    ...buildHomeMainSectionsViewModel(contentData, newsData, eventsData, partnersData),
  }
}
