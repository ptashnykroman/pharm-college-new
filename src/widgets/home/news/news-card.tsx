...
import {
  ...
  getNewsTagTone,
  newsToneStyles,
} from "@/widgets/home/news/news-utils";

type NewsCardProps = {
  item: HomePageViewModel["news"][number];
  index: number;
  page: number;
};

export function NewsCard({ item, index, page }: NewsCardProps) {
  ...
  return (
    ...
    >
      ...
      <div className="relative flex flex-1 flex-col p-6">
        <h3 className="text-lg font-bold leading-snug text-foreground transition-smooth group-hover:text-primary">
          {item.title}
        </h3>
