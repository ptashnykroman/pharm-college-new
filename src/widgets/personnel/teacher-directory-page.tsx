...
function TeacherCard({ teacher, index }: { teacher: TeacherCardViewModel; index: number }) {
  ...
  return (
    <Link
      href={teacher.href}
      className={cn(
        'group relative flex h-full min-h-[260px] flex-col overflow-hidden rounded-[2rem] border border-border/70 bg-white shadow-soft transition hover:-translate-y-1.5 hover:shadow-card lg:flex-row',
        isReversed && 'xl:flex-row-reverse',
      )}
    >
      <div className="relative shrink-0 overflow-hidden bg-gradient-to-br from-primary-deep via-primary to-primary-glow lg:w-[42%]">
        {teacher.photo ? (
          <Image
          ...
          <div className="flex h-full min-h-[220px] items-center justify-center text-6xl font-black text-white/90">
            {getInitials(teacher.name)}
          </div>
        )}

        {/* <div className="absolute left-4 top-4 rounded-full bg-white/90 px-3 py-1 text-[11px] font-bold uppercase tracking-[0.2em] text-primary shadow-soft">
          Профіль
        </div> */}
      </div>
      ...
      <div className="flex flex-1 flex-col p-3 md:p-5">
        ...
        <p className="mt-4 overflow-hidden text-sm leading-5 text-foreground/75 [display:-webkit-box] [-webkit-box-orient:vertical] [-webkit-line-clamp:4]">
    ...
    <section className="relative overflow-hidden bg-gradient-soft py-12 md:py-16">
      ...
      <div className="container relative mx-auto px-4 md:px-6">
        ...
        <div className="w-full max-w-sm">
          <label className="mb-1 block text-sm font-bold text-foreground/65">ЦК</label>
          <Select value={selectedCommission} onValueChange={setSelectedCommission}>
            <SelectTrigger className="!h-12 w-full rounded-full border-primary/15 bg-white px-4 shadow-soft cursor-pointer">
        ...
        {filteredTeachers.length ? (
          ...
          <div className="mt-10 rounded-[2rem] border border-dashed border-border bg-white/80 px-6 py-12 text-center shadow-soft">
