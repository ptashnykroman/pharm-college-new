...
const appButtonVariants = cva("", {
  variants: {
    ...
    size: {
      default:
        "min-h-12 px-6 py-2 has-data-[icon=inline-start]:pl-6 has-data-[icon=inline-end]:pr-5",
      sm: "min-h-10 px-5 has-data-[icon=inline-start]:pl-5 has-data-[icon=inline-end]:pr-4",
...
function AppButton({
  ...
  const content = (
    <>
      {Icon && iconPosition === "left" ? (
        <Icon data-icon="inline-start" className="transition-bounce" />
      ) : null}
      ...
      {Icon && iconPosition === "right" ? (
        <Icon
          ...
          className={cn(
            "transition-bounce",
            variant === "surface" && "group-hover/button:translate-x-1",
          )}
        />
      ) : null}
    </>
  ...
  const resolvedClassName = cn(
    appButtonVariants({ shape, size, variant, width }),
    className,
  );
