...
import type { ImageLightboxSlide, ImageLightboxSource } from '@/components/shared/image-lightbox'
import { cn } from '@/lib/utils'
import type { ResolvedImageSource, ResolvedImageWithSources } from '@/shared/lib/media'
import { Maximize2 } from 'lucide-react'

const LIGHTBOX_IMAGE_WIDTHS = [16, 32, 48, 64, 96, 128, 256, 384, 640, 750, 828, 1080, 1200, 1920, 2048, 3840]

const LIGHTBOX_IMAGE_QUALITY = 75

const loadImageLightbox = () => import('@/components/shared/image-lightbox')

const ImageLightbox = dynamic(() => loadImageLightbox().then((mod) => mod.ImageLightbox), { ssr: false })
...
function pickSourceForWidth(sources: readonly ResolvedImageSource[], targetWidth: number) {
  return sources.find((source) => source.width >= targetWidth) ?? sources[sources.length - 1]
}
...
function buildLightboxSlide(image: ResolvedImageWithSources): ImageLightboxSlide {
  ...
  const responsiveSources = LIGHTBOX_IMAGE_WIDTHS.filter((width) => width < largestSource.width).map((width) => {
    const source = pickSourceForWidth(sources, width)
    const height = Math.round((largestSource.height / largestSource.width) * width)
...
export function ImageLightboxGallery({
  ...
  const slides = useMemo(() => items.map((item) => buildLightboxSlide(item.image)), [items])
  ...
  return (
    <>
      <div className={cn('mt-5 grid gap-4 sm:grid-cols-2 xl:grid-cols-3', gridClassName)}>
      ...
      {shouldRenderLightbox ? (
        <ImageLightbox open={lightboxOpen} close={() => setLightboxOpen(false)} index={lightboxIndex} slides={slides} />
      ) : null}
    </>
  )
}
